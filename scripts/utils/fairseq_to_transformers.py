...
"""Convert RoBERTa checkpoint to huggingface checkpoints."""
...
SAMPLE_TEXT = [
    ...
    "This script converts checkpoints from fairseq to ones that can be used by transformers.",
    "It is a lovely day. It is a lovely day today.",
]


def from_pretrained(
    roberta_checkpoint_dir: str,
    roberta_checkpoint_name: str,
    roberta_data_dir: str,
    roberta_user_dir: str,
):
    kwargs = {}
    if roberta_user_dir is not None:
        kwargs["user_dir"] = roberta_user_dir
    kwargs["load_checkpoint_name"] = True
    kwargs["gpt2_encoder_json"] = fairseq.data.encoders.gpt2_bpe.DEFAULT_ENCODER_JSON
    kwargs["gpt2_vocab_bpe"] = fairseq.data.encoders.gpt2_bpe.DEFAULT_VOCAB_BPE
    kwargs["bpe"] = "gpt2"

    x = hub_utils.from_pretrained(
        roberta_checkpoint_dir,
        checkpoint_file=roberta_checkpoint_name,
        data_name_or_path=roberta_data_dir,
        **kwargs,
    )
...
def convert_roberta_checkpoint_to_pytorch(
    ...
    if classification_head:
        ...
        model.lm_head.layer_norm.weight = (
            roberta.model.decoder.lm_head.layer_norm.weight
        )
    ...
    for text in SAMPLE_TEXT:
        ...
        input_ids: torch.Tensor = roberta.encode(text).unsqueeze(0)  # batch of size 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path the fairseq checkpoint directory containing the checkpoint.",
    )
    parser.add_argument(
        "--roberta_checkpoint_name",
        default="model.pt",
    ...
    parser.add_argument(
        ...
        default=".",
        type=str,
        help="Path to the folder containing dict.txt.",
    )
    parser.add_argument(
        "--roberta_user_dir",
        default=None,
        type=str,
        help="Path to the user dir used in training the checkpoint.",
    )
    parser.add_argument(
    ...
    convert_roberta_checkpoint_to_pytorch(
        args.roberta_checkpoint_dir,
        args.roberta_checkpoint_name,
        args.roberta_data_dir,
        args.roberta_user_dir,
